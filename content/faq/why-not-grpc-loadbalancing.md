...
## 分析
grpc 是基于 http2 的长连接，多次请求复用同一个连接。在 istio 中，默认会对 grpc 的请求进行请求级别的负载均衡，如果要让 grpc 在请求级别进行负载均衡，核心就是让 istio 正确识别是 grpc 协议，不要配置成 tcp，用 tcp 就只能在连接级别进行负载均衡了，通常这也是造成负载不均的原因。
